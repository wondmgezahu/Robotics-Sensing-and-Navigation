...
path_to_bag = os.path.join(path_to_src, 'Data/gps_occluded/gps_bag_0.db3')
...
plt.title('Scaled Northing vs Easting Data (Occluded)')
plt.xlabel('Northing')
plt.ylabel('Easting')
plt.savefig('northing_occluded.png')
plt.show()

# Calculating position error
known_lat = 42.338548
known_long = -71.088760
pos_error = []
...
plt.savefig('hist_occluded.png')
plt.show()

# Scatterplot of altitude vs time
df.plot.scatter(x='utc',y='altitude')
plt.title('Altitude vs Time (Occluded)')
plt.xlabel('Time (UTC)')
plt.ylabel('Altitude')
plt.savefig('altitude_occluded.png')
plt.show()
